...
class UserServiceServicer(IUserServiceServicer):
    ...
    def GetUser(self, request, context):  # type: ignore
        user = self._repository.get(UserId(request.id))
        return UserResponse(id=user.id_, name=user.name)

    def CreateUser(self, request, context):  # type: ignore
        user = User(self._repository.nextId(), request.name)
        self._repository.save(user)
        return UserResponse(id=user.id_, name=user.name)

    def UpdateUser(self, request, context):  # type: ignore
        user = self._repository.get(UserId(request.id))
        user.updateName(request.name)
        self._repository.save(user)
        return UserResponse(id=user.id_, name=user.name)

    def DeleteUser(self, request, context):  # type: ignore
        user = self._repository.get(UserId(request.id))
        self._repository.delete(UserId(request.id))
        return UserResponse(id=user.id_, name=user.name)

    def ListUsers(self, request, context):  # type: ignore
